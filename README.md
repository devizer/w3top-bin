...
For disk benchmark it is recommended to install latest fio and libaio. w3top comes with precompiled binaries of fio for all the supported architectures (x64, arm32 and arm64), but system fio takes precedence. After upgrading system fio or system libaio fio metadata cache should be flushed: ``rm ~/.cache/fio``
